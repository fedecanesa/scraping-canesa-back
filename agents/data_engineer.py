...
def scrape_website(
    ...
    if not token:
        raise ValueError(
            "Se requiere APIFY_API_TOKEN. Configúralo en el entorno o en .env"
        )
...
def clean_scraped_data(
    ...
    if not api_key:
        raise ValueError(
            "Se requiere OPENAI_API_KEY. Configúralo en el entorno o en .env"
        )
...
- Devuelve ÚNICAMENTE el texto limpio, sin comentarios ni explicaciones"""
    ...
    for i, item in enumerate(scraped_items, start=1):
        ...
        if not content or not content.strip():
            cleaned.append({"url": url, "cleaned_text": "", "metadata": metadata})
            continue
...
def run_scraper_and_clean(
    ...
    return clean_scraped_data(scraped_items=scraped, model=model)
